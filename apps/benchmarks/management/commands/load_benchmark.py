...
class Command(BaseCommand):
    ...
    def load_benchmark(self, slug, num_samples=0, force=False):
        ...
        # Insert in small batches to avoid holding the SQLite write-lock
        # for too long (prevents "database is locked" errors).
        BATCH = 200
        batch = []
        saved = 0
        for qdata in questions_data:
            ...
            if len(batch) >= BATCH:
                ...
                saved += len(batch)
                batch = []
                self.stdout.write(f'\r  Saved {saved}/{len(questions_data)} questions', ending='')
                self.stdout.flush()
        if batch:
            BenchmarkQuestion.objects.bulk_create(batch, ignore_conflicts=True)
